...
!developmentChains.includes(network.name)
    ...
    : describe("Dynamic SVG NFT", function () {
          let deployer, dynamicNft: DynamicSvgNft, mockV3Aggregator: MockV3Aggregator
          ...
          describe("constructor", () => {
              it("sets value correctly", async () => {
                  ...
                  assert.equal(lowSvg.toString(), lowSVGImageuri)
                  assert.equal(highSvg.toString(), highSVGimageUri)
                  assert.equal(priceFeed, await mockV3Aggregator.getAddress())
                  console.log("latestRoundData: " + (await mockV3Aggregator.latestRoundData()))
              })
          })

          describe("mintNFT", () => {
              it("emit an event createdNFT", async () => {
              ...
              it("shift the token url to low", async () => {
                  const highValue = await ethers.parseEther("1000000000")
